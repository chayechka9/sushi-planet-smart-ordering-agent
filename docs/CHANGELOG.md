...
- В `package-lock.json` транзитивный `fast-uri` обновлён с `4.1.2` до
  `4.2.1`, а вложенная зависимость AJV — с `3.1.5` до `3.1.8`; lifecycle
  scripts были отключены. `package.json`, исходный код, конфигурация, тесты и
  архитектура не менялись.
- `npm audit --omit=dev` завершился с `0` уязвимостей; ранее выявленная
  high-уязвимость `fast-uri` устранена.
- Проверки: `npm ls fast-uri --all`; `npm test` — 389 тестов в 39 файлах
  прошли; `npm run typecheck`; `npm run build`; `git diff --check` — успешно.
- Telegram, OpenAI, SumUp, Poster, checkout, webhook, реальные заказы и платежи
  не запускались.
- Result: complete — выполнено только минимальное транзитивное security-
  обновление без изменения бизнес-логики или API.
- Commit: текущий коммит, содержащий эту запись.
...
- Controlled Telegram delivery flow больше не читает одну общую
  `TELEGRAM_DELIVERY_FEE_CENTS`. Он использует отдельный
  `LocalDeliveryTariffResolver`, который получает район из существующего поля
  `city` и Eircode из `postalCode`, нормализует регистр и пробелы и возвращает
  цену только при одном точном совпадении разрешённого правила.
- Источник тарифов — ignored локальный
  `.telegram-delivery-tariffs.json`. Добавлены пустой tracked example и JSON
  Schema версии `1`; запись требует `feeCents` и хотя бы один selector
  `district` или `eircode`. Реальные и вымышленные ресторанные зоны или тарифы
  в конфигурацию не добавлялись.
- Missing/пустой/повреждённый config, неожиданные поля, нецелая, отрицательная
  или unsafe цена, неизвестная зона и более одного совпавшего правила
  безопасно отклоняются. Prefix/fallback matching отсутствует, поэтому цена не
  угадывается.
- Ошибка resolver преобразуется существующим conversation core в отдельный
  безопасный `delivery_unavailable`; Telegram отвечает
  `Доставка в эту зону пока недоступна.`. При отказе адрес и стоимость доставки
  не сохраняются, а состояние продолжает требовать адрес.
- Точечные synthetic tests покрывают нормализацию, точное совпадение района и
  Eircode, отсутствие prefix match, неизвестную зону, пустую таблицу,
  невалидные тарифы, missing/malformed config, неоднозначное совпадение и
  отсутствие изменения SQLite state при отказе. Test-only fixtures не являются
  данными Sushi Planet.
- `src/server.ts`, checkout, payment, Poster, SumUp, OpenAI и production wiring
  не менялись. Telegram polling, реальные заказы, платежи и любые внешние
  запросы не запускались.
- Проверки: точечные тесты — 45 тестов в 3 файлах прошли; `npm test` — 352
  теста в 36 файлах прошли; `npm run typecheck`; `npm run build`;
  `git diff --check` — успешно.
- Result: complete — локальный resolver и fail-closed Telegram wiring готовы;
  реальные тарифы остаются пустыми до получения утверждённой рестораном
  таблицы район/Eircode → цена в евроцентах.
- Commit: текущий коммит, содержащий эту запись.

## 14 сентября 2026

### Изменение корзины и получение заказа в controlled Telegram flow

- Deterministic Telegram interpreter расширен bounded-командами
  `/remove <номер из корзины> [количество]`, `/pickup`, `/delivery`,
  `/name <имя>`, `/phone <телефон>`,
  `/address <улица> | <город> | <индекс>` и `/review`. Все принятые команды
  преобразуются в уже существующие allowlisted `ConversationAgentCommand`;
  отдельная корзина, расчёт сумм или новый order workflow не создавались.
- `/cart` теперь нумерует строки в том же порядке, который использует
  `/remove`. Без количества удаляется вся строка; положительное количество
  уменьшает существующую quantity через core `set_quantity`, а количество,
  равное текущему, использует core `remove_item`. Неизвестный номер, `0`,
  количество больше имеющегося и лишние аргументы безопасно отклоняются без
  изменения заказа.
- `/pickup` фиксирует самовывоз и использует существующее правило core, которое
  удаляет ранее сохранённый delivery address. Для готовности самовывоза нужны
  корзина, имя и телефон, но адрес не требуется. `/delivery` требует корзину,
  имя, телефон и успешно сохранённый адрес.
- Имя, телефон и три структурированные части адреса проходят bounded локальную
  валидацию до вызова core. Некорректные значения и `/address` до `/delivery`
  возвращают только безопасную инструкцию; значения клиента не отражаются в
  ответах Telegram или aggregate runtime events.
- Delivery address применяется существующим order core вместе с явно заданной
  локальной стоимостью `TELEGRAM_DELIVERY_FEE_CENTS`. Пустое или невалидное
  значение не заменяется выдуманным тарифом: core отклоняет адрес как
  недоступную доставку и не сохраняет его. Эта настройка не включает runtime и
  не ослабляет обязательные gates polling runner.
- `/review` отображает нумерованный состав корзины, EUR total из существующего
  order core, выбранный способ получения и полный список незаполненных
  обязательных полей. При заполнении всех полей сообщается только локальная
  готовность данных; checkout, оплата и создание заказа не запускаются.
- SQLite identity/message duplicate guards не менялись. Fake-transport tests
  подтверждают partial и full remove, duplicate без повторного изменения,
  pickup без адреса, delivery с локальной synthetic fee, валидацию команд и
  полей, отсутствие fallback fee, сохранение customer/fulfilment/address/cart
  после закрытия и повторного открытия temporary SQLite, а также отсутствие
  customer values в исходящих сообщениях.
- `src/server.ts` не изменён и остаётся health-only. Telegram polling, OpenAI,
  SumUp, Poster, ChoiceQR, checkout и внешние запросы в этой задаче не
  запускались; `.env` и фактический menu snapshot не открывались.
- Проверки: `npm test` — 342 теста в 35 файлах прошли;
  `npm run typecheck`; `npm run build`; `git diff --check` — успешно.
- Result: complete — локальный controlled Telegram flow умеет менять корзину,
  собирать bounded данные получения и восстанавливать их из SQLite; реальный
  тест с ботом остаётся отдельным внешним действием.
- Commit: текущий коммит, содержащий эту запись.
